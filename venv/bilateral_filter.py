...
def bl_pixel(image, i, j, sigma_d, sigma_r):
    c = 0
    s = 0
...
def bilateral(image, sigma_d, sigma_r):
    ...
    for i in range(1, w-1):
        for j in range(1, h-1):
            n_image[i, j] = bl_pixel(image, i, j, sigma_d, sigma_r)
    return n_image

IMG = 'original_image_grayscale.bmp'
...
cv2.imwrite("gpu.bmp", gpu_result.astype(np.uint8))
...
cv2.imwrite("cpu.bmp", cpu_result)
